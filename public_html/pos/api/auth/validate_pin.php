...
/**
 * POS Auth - Validate PIN (manager approvals)
 * Body: { pin }
 */
...
$in = read_input();
$pin = $in['pin'] ?? '';
if ($pin === '') respond(false, 'Missing pin', 400);
...
try {
    ...
    // Approver roles — extend as needed to match your role keys
    $allowed = ['manager', 'admin', 'owner', 'super_admin'];
    if (!in_array($role, $allowed, true)) {
        respond(false, 'Permission denied', 403);
    }
