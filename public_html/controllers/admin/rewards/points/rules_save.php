...
// /controllers/admin/rewards/points/rules_save.php
...
try {
  db_tx(function(PDO $pdo) use ($TENANT_ID, $earn) {
    $st = $pdo->prepare("SELECT id FROM loyalty_programs WHERE tenant_id=:t AND program_type='points' LIMIT 1");
    ...
    if ($row) {
      ...
      $ins = $pdo->prepare("
        ...
        VALUES
          (:t,'points','Points Program',:e,'{}',1,NOW(),NOW())
      ");
