...
<head>
  ...
  <title>Rewards · Points · Ledger</title>
...
<div class="container mt-4">
  ...
  <h1 class="h3 mb-3">Rewards · Points · Ledger</h1>
  <?php
$pdo = db();
$from = $_GET['from'] ?? null;
$to   = $_GET['to'] ?? null;
$params = [':t'=>$tenantId];
$where = "tenant_id=:t";
if ($from && preg_match('/^\d{4}-\d{2}-\d{2}$/',$from)) { $where .= " AND DATE(created_at) >= :from"; $params[':from']=$from; }
if ($to && preg_match('/^\d{4}-\d{2}-\d{2}$/',$to))     { $where .= " AND DATE(created_at) <= :to";   $params[':to']=$to; }
$rows=[];
try{
  $stmt=$pdo->prepare("SELECT id, customer_id, order_id, type, points_delta, reason, created_at FROM loyalty_ledger WHERE $where ORDER BY id DESC LIMIT 500");
  ...
  <div class="card-body">
    <div class="table-responsive">
      <table class="table table-sm table-striped align-middle">
        <thead><tr>
          <th>ID</th><th>Customer</th><th>Order</th><th>Type</th><th class="text-end">Points</th><th>Reason</th><th>When</th>
        </tr></thead>
        <tbody>
        <?php if(!$rows): ?>
          <tr><td colspan="7" class="text-muted">No transactions.</td></tr>
        <?php else: foreach($rows as $r): ?>
          <tr>
            ...
            <td><?=h($r['type'])?></td>
            <td class="text-end"><?=h((string)$r['points_delta'])?></td>
            <td><?=h($r['reason'] ?? '')?></td>
