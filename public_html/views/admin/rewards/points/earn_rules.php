...
<head>
  ...
  <title>Rewards · Points · Earn Rules</title>
...
<div class="container mt-4">
  ...
  <h1 class="h3 mb-3">Rewards · Points · Earn Rules</h1>
  <?php
$pdo = db();
$program = null;
try{
  $stmt=$pdo->prepare("SELECT id,name,earn_rule_json FROM loyalty_programs WHERE tenant_id=:t AND program_type='points' ORDER BY updated_at DESC LIMIT 1");
  $stmt->execute([':t'=>$tenantId]); $program=$stmt->fetch(PDO::FETCH_ASSOC);
}catch(Throwable $e){ $program=null; }
$pretty = '{}';
if ($program && !empty($program['earn_rule_json'])) {
  $pretty = json_encode(json_decode((string)$program['earn_rule_json'], true), JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES);
  if ($pretty === null) { $pretty = (string)$program['earn_rule_json']; }
}
?>
<div class="card shadow-sm">
  <div class="card-body">
    <h2 class="h6">Earn Rules (latest program)</h2>
